...
A **full-height side panel** that mirrors the entire page structure.

Every visible element on the page has a corresponding entry in the panel.
The panel explains what each element means, why it matters, what it connects
to, and what the user should do about it.

### What the user sees

A panel to the right of the wizard content (or modal content). It shows the
**entire page structure** as a tree of assistant content — step context at
top, sections, sub-sections, instances, fields, all the way down.

When the user hovers or focuses an element, that element's entry in the
panel **expands** to show deeper detail. Other entries stay concise.
...
1. **Full page mirror** — every visible element has an assistant entry
2. **Hierarchical** — nesting in the panel matches nesting on the page
3. **Always present** — the panel shows everything, not just the focused item
4. **Focus expands** — hovered/focused element gets deeper detail
...
- **Everything shows** — no category priority needed, everything is visible
- **Focus only controls depth** — expand/collapse, not show/hide
...
The engine renders each node as a div with depth-based indentation:
...
function renderNode(node, depth, panelEl) {
    ...
    for (const child of node.children) {
        renderNode(child, depth + 1, panelEl);
    }
}
```
...
Two approaches depending on scroll context:

**For wizard (same scroll container):**
Panel and content are in the same flex layout within the scrolling body.
They scroll together naturally. The engine just needs to ensure the expanded
node is visible.

**For modals (modal has its own scroll):**
The modal body scrolls independently. The engine listens for scroll events
on the modal body and adjusts the panel's scroll position to keep the
focused node aligned.

```javascript
function syncScroll(containerEl, panelEl) {
    containerEl.addEventListener('scroll', () => {
        const focusedNode = panelEl.querySelector('.assistant-node.active');
        if (focusedNode) {
...
| L8 State | **Eliminated for V1.** No need to track visited, dismissed, rejected. Everything is always visible. |
