    ...
    *,
    ...
    def _run():
        ...
        try:
            ...
            summary = f"Test replay: {suite.name} — {passed}/{total} passed"
            if failed:
                summary += f", {failed} failed"
            emit_event(
                "cdp_test.replay.completed",
                summary=summary,
                correlation_id=f"cdp_test:{run_id[:8]}",
                status="ok" if result.status == "passed" else "error",
                duration_ms=result.duration_ms or 0,
                detail={
                    "run_id": run_id, "suite_id": suite.id,
                    "suite_name": suite.name, "status": result.status,
                    "passed": passed, "failed": failed,
                    "skipped": result.skipped_steps, "total": total,
