...
def _test_target_reachable(
    target_host: str, target_port: int, timeout: float = 1.0,
) -> bool:
...
def _ensure_windows_routing(
    ...
    **netsh portproxy**: Windows forwards <hostname.local IP>:<port> →
    ...
    script_lines = [
        ...
        "",
        f"Write-Host '[WSL Channel] Setting up portproxy: {listen_addr}:9222-9232 -> 127.0.0.1'",
        f"Write-Host '[WSL Channel] {num_ports} ports to configure'",
        "Write-Host ''",
        "$errors = 0",
    ]
    for i, p in enumerate(range(port_start, port_end + 1), 1):
        script_lines.extend([
            ...
            "  if ($LASTEXITCODE -ne 0) { Write-Host \"  WARNING: delete 0.0.0.0 rule: $r\" }",
            "  else { Write-Host '  Old 0.0.0.0 rule removed' }",
            "} else { Write-Host '  No old 0.0.0.0 rule (OK)' }",
            # Check and clean existing specific-IP rule
            f"$existing2 = netsh interface portproxy show v4tov4 | Select-String '{p}.*{listen_addr}'",
            "if ($existing2) {",
            f"  Write-Host '  Removing existing {listen_addr} rule...'",
            f"  $r = netsh interface portproxy delete v4tov4 listenport={p} listenaddress={listen_addr} 2>&1",
            "  if ($LASTEXITCODE -ne 0) { Write-Host \"  WARNING: delete existing rule: $r\" }",
            "  else { Write-Host '  Existing rule removed' }",
            "} else { Write-Host '  No existing rule (OK)' }",
            # Add new rule
            f"Write-Host '  Adding: {listen_addr}:{p} -> 127.0.0.1:{p}'",
            f"$r = netsh interface portproxy add v4tov4 listenport={p} listenaddress={listen_addr} connectport={p} connectaddress=127.0.0.1 2>&1",
            "if ($LASTEXITCODE -ne 0) {",
            f"  Write-Host \"  ERROR: Failed to add rule for port {p}: $r\"",
            "  $errors++",
            f"}} else {{ Write-Host '  OK: port {p} configured' }}",
            "",
        ])
    script_lines.extend([
        "Write-Host ''",
        ...
        "Write-Host ''",
        "if ($errors -gt 0) {",
        "  Write-Host \"[WSL Channel] DONE with $errors error(s)\" -ForegroundColor Yellow",
        "  exit 1",
        "} else {",
        f"  Write-Host '[WSL Channel] DONE: {num_ports} ports configured successfully' -ForegroundColor Green",
        "  exit 0",
        "}",
    ])
    script_content = "\r\n".join(script_lines) + "\r\n"
    ...
    try:
        ...
        # Log PS output for troubleshooting
        if r.stdout.strip():
            for line in r.stdout.strip().splitlines():
                logger.info("[netsh-setup] %s", line)
        if r.returncode != 0:
            logger.warning(
...
def count_portproxy_rules() -> int:
    """Return the number of active netsh portproxy rules for CDP ports 9222–9232.

    Non-elevated — just reads the current portproxy table.
    ...
    try:
        ...
        return sum(
            ...
            if any(f" {p} " in line or line.strip().startswith(str(p))
...
def remove_all_portproxy_rules() -> bool:
    """Remove all netsh portproxy rules for CDP ports 9222–9232.

    Parses the live portproxy table to find every listen address bound to
    ports in the CDP range, then deletes each one.  Uses elevated PowerShell
    (UAC prompt).

    Returns True on success, False on failure or UAC cancellation.
    """
    script_lines = [
        "# Auto-generated by WSL Channel — remove all CDP portproxy rules",
        "$ErrorActionPreference = 'Continue'",
        "",
        "Write-Host '[WSL Channel] Removing portproxy rules for CDP ports 9222-9232'",
        "$removed = 0",
        "$errors  = 0",
        "",
        "# Parse live portproxy table — format: <addr>  <port>  <addr>  <port>",
        "$lines = netsh interface portproxy show v4tov4",
        "foreach ($line in $lines) {",
        "  if ($line -match '^\\s*(\\S+)\\s+(\\d+)\\s+') {",
        "    $addr = $matches[1]",
        "    $port = [int]$matches[2]",
        "    if ($port -ge 9222 -and $port -le 9232) {",
        "      Write-Host \"  Removing: $addr`:$port\"",
        "      $r = netsh interface portproxy delete v4tov4 `",
        "        listenport=$port listenaddress=$addr 2>&1",
        ...
        "Write-Host ''",
        "if ($errors -gt 0) {",
        "  Write-Host \"[WSL Channel] Done: $removed removed, $errors error(s)\" -ForegroundColor Yellow",
        "  exit 1",
        "} else {",
        "  Write-Host \"[WSL Channel] Done: $removed rule(s) removed\" -ForegroundColor Green",
        "  exit 0",
        "}",
    ]
    script_content = "\r\n".join(script_lines) + "\r\n"

    script_path_wsl = "/mnt/c/Windows/Temp/wsl_cdp_portproxy_del.ps1"
    script_path_win = "C:\\Windows\\Temp\\wsl_cdp_portproxy_del.ps1"
    ...
    try:
        ...
        if r.stdout.strip():
            for line in r.stdout.strip().splitlines():
                logger.info("[portproxy-del] %s", line)

        if r.returncode != 0:
            logger.warning(
                "Portproxy removal failed (rc=%d): %s",
                r.returncode, r.stderr.strip()[:300],
            )
            return False

        logger.info("All CDP portproxy rules removed (9222-9232)")
        return True

    except subprocess.TimeoutExpired:
        logger.warning("Portproxy removal timed out (UAC cancelled?)")
        return False
    except Exception as exc:
        logger.warning("Portproxy removal failed: %s", exc)
        return False
...
class NetshTunnel:
    ...
    def start(self) -> bool:
        ...
        script_content = (
            "$ErrorActionPreference = 'Continue'\r\n"
            f"Write-Host '[NetshTunnel] Creating portproxy: {listen_addr}:{self._local_port} -> 127.0.0.1:{self._target_port}'\r\n"
            "\r\n"
            "# Check for existing rule\r\n"
            f"$existing = netsh interface portproxy show v4tov4 | Select-String '{self._local_port}.*{listen_addr}'\r\n"
            "if ($existing) {\r\n"
            f"  Write-Host '  Removing existing rule for port {self._local_port}...'\r\n"
            f"  $r = netsh interface portproxy delete v4tov4 listenport={self._local_port} listenaddress={listen_addr} 2>&1\r\n"
            "  if ($LASTEXITCODE -ne 0) { Write-Host \"  WARNING: delete failed: $r\" }\r\n"
            "  else { Write-Host '  Old rule removed' }\r\n"
            "} else { Write-Host '  No existing rule (OK)' }\r\n"
            "\r\n"
            "# Add new rule\r\n"
            f"Write-Host '  Adding: {listen_addr}:{self._local_port} -> 127.0.0.1:{self._target_port}'\r\n"
            f"$r = netsh interface portproxy add v4tov4 listenport={self._local_port} listenaddress={listen_addr} connectport={self._target_port} connectaddress=127.0.0.1 2>&1\r\n"
            "if ($LASTEXITCODE -ne 0) {\r\n"
            "  Write-Host \"  ERROR: $r\"\r\n"
            "  exit 1\r\n"
            "}\r\n"
            ...
            "Write-Host '--- Current portproxy rules ---'\r\n"
            "netsh interface portproxy show v4tov4\r\n"
            f"Write-Host '[NetshTunnel] OK: port {self._local_port} configured'\r\n"
            "exit 0\r\n"
        )
        ...
        try:
            ...
            if r.stdout.strip():
                for line in r.stdout.strip().splitlines():
                    logger.info("[netsh-start] %s", line)
    ...
    def stop(self) -> None:
        ...
        script_content = (
            "$ErrorActionPreference = 'Continue'\r\n"
            f"Write-Host '[NetshTunnel] Removing portproxy for port {self._local_port} (listen={listen_addr})'\r\n"
            "\r\n"
            "# Check if rule exists before deleting\r\n"
            f"$existing = netsh interface portproxy show v4tov4 | Select-String '{self._local_port}.*{listen_addr}'\r\n"
            "if (-not $existing) {\r\n"
            f"  Write-Host '  No portproxy rule found for port {self._local_port} — nothing to remove'\r\n"
            "  exit 0\r\n"
            "}\r\n"
            "\r\n"
            f"Write-Host '  Deleting rule: {listen_addr}:{self._local_port}'\r\n"
            f"$r = netsh interface portproxy delete v4tov4 listenport={self._local_port} listenaddress={listen_addr} 2>&1\r\n"
            "if ($LASTEXITCODE -ne 0) {\r\n"
            "  Write-Host \"  ERROR: delete failed: $r\"\r\n"
            "  exit 1\r\n"
            "}\r\n"
            f"Write-Host '[NetshTunnel] OK: port {self._local_port} rule removed'\r\n"
            "exit 0\r\n"
        )
        ...
        try:
            ...
            if r.stdout.strip():
                for line in r.stdout.strip().splitlines():
                    logger.info("[netsh-stop] %s", line)

            if r.returncode != 0:
                logger.warning(
                    "NetshTunnel: remove failed (rc=%d): %s",
                    r.returncode, r.stderr.strip()[:300],
                )
            else:
                logger.info(
